...
const pageTitle = computed(() => (isEditMode.value ? 'แก้ไขค่าใช้จ่ายอื่น ๆ' : 'เพิ่มค่าใช้จ่ายอื่น ๆ'));
...
/**
 * ดึงข้อมูลค่าใช้จ่ายอื่น ๆ สำหรับแก้ไข
 */
const fetchExpenseData = async () => {
    try {
        ...
        const response = await api.getMasterExpenseById(route.params.id);
        ...
        router.push({ name: 'other-expenses' });
    } finally {
...
const confirmSave = async () => {
    ...
    try {
        isSaving.value = true;
        showLoading(isEditMode.value ? 'กำลังบันทึกข้อมูล...' : 'กำลังสร้างค่าใช้จ่ายอื่น ๆ...');
        ...
        let response;
        if (isEditMode.value) {
            response = await api.updateMasterExpense(route.params.id, payload);
        } else {
            response = await api.createMasterExpense(payload);
        }
        ...
        if (response.success) {
            toast.add({
                severity: 'success',
                summary: 'สำเร็จ',
                detail: isEditMode.value ? 'บันทึกข้อมูลเรียบร้อยแล้ว' : 'สร้างค่าใช้จ่ายอื่น ๆ เรียบร้อยแล้ว',
                life: 3000
            });
...
<template>
    <div class="card">
        <div class="flex flex-col gap-4">
            ...
            <form v-else @submit.prevent="handleSubmit" class="flex flex-col gap-6">
                <!-- Row 1: รหัสค่าใช้จ่าย และ ชื่อค่าใช้จ่าย -->
                <div class="flex flex-col md:flex-row gap-6">
                    <div class="flex flex-col gap-2 md:w-1/3">
                        ...
                        <InputText id="code" v-model="formData.code" placeholder="กรอกรหัสค่าใช้จ่าย" :invalid="isCodeInvalid" fluid />
                        <small v-if="isCodeInvalid" class="text-red-500 dark:text-red-400 flex items-center gap-1">
                            <i class="pi pi-exclamation-circle"></i>
                            กรุณากรอกรหัสค่าใช้จ่าย
                        </small>
                    </div>
                    ...
                    <div class="flex flex-col gap-2 md:w-2/3">
                        ...
                        <InputText id="name" :modelValue="getThaiName()" @update:modelValue="setThaiName" placeholder="กรอกชื่อค่าใช้จ่าย" :invalid="isNameInvalid" fluid />
                        <small v-if="isNameInvalid" class="text-red-500 dark:text-red-400 flex items-center gap-1">
                            <i class="pi pi-exclamation-circle"></i>
                            กรุณากรอกชื่อค่าใช้จ่าย
                        </small>
                    </div>
                </div>
    ...
    <DialogForm
        ...
        :textContent="isEditMode ? 'คุณต้องการบันทึกการแก้ไขค่าใช้จ่ายอื่น ๆ หรือไม่?' : 'คุณต้องการสร้างค่าใช้จ่ายอื่น ๆ ใหม่หรือไม่?'"
