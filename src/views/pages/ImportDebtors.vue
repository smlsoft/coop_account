...
// Constants
const TEMPLATE_URL = '/demo/file/template_debtors.xlsx';
...
const downloadTemplate = () => {
    ...
    link.download = 'template_debtors.xlsx';
    link.click();
};
...
const processFile = (file) => {
    ...
    reader.onload = (e) => {
        try {
            ...
            const requiredHeaders = ['CODE', 'NAME', 'AR_TYPE', 'TAXID', 'BRANCH_TYPE', 'BRANCH_CODE', 'ADDRESS', 'TELEPHONE'];
            ...
            const parsedData = dataRows
                ...
                .map((row) => ({
                    ...
                    personaltype: parseInt(row[headerMap['AR_TYPE']]) || 1,
                    taxid: row[headerMap['TAXID']] ? String(row[headerMap['TAXID']]).trim() : '',
                    customertype: parseInt(row[headerMap['BRANCH_TYPE']]) || 1,
...
const verifyData = (data) => {
    ...
    uniqueCodes.forEach((code) => {
        ...
        if (duplicates.length > 1) {
            ...
            if (!item.code) {
                errors.push({
                    word: 'รหัสลูกหนี้ไม่สามารถเว้นว่างได้',
                    code: '-'
                });
            }
            ...
            if (!item.names[0]?.name) {
                errors.push({
                    word: 'ชื่อลูกหนี้ไม่สามารถเว้นว่างได้',
                    code: item.code
                });
            }
        }
...
const confirmSave = async () => {
    ...
    try {
        ...
        const response = await api.importDebtors(importData.value);
...
<template>
    <div class="card">
        <div class="flex flex-col gap-4">
            ...
            <div class="flex justify-between items-center pb-4 border-b border-surface">
                <div>
                    <div class="text-2xl font-bold text-surface-900 dark:text-surface-0">นำเข้าข้อมูลลูกหนี้</div>
                    <p class="text-surface-600 dark:text-surface-400 mt-1">นำเข้าข้อมูลลูกหนี้จากไฟล์ Excel</p>
                </div>
            </div>
            ...
            >
                ...
                <Column field="code" header="รหัสลูกหนี้" :sortable="false" style="min-width: 10rem">
                    <template #body="{ data }">
                        <span class="text-primary font-semibold">{{ data.code }}</span>
                    </template>
                </Column>

                <Column header="ชื่อลูกหนี้" :sortable="false" style="min-width: 15rem">
                    <template #body="{ data }">
                ...
                <Column field="personaltype" header="ประเภทบุคคล" :sortable="false" style="min-width: 10rem">
                    <template #body="{ data }">
                        <Tag :value="getPersonalTypeLabel(data.personaltype)" :severity="data.personaltype === 2 ? 'info' : 'success'" />
                    </template>
                </Column>
            ...
            >
                <div class="flex flex-col items-center justify-center py-12 px-8">
                    ...
                    <p class="text-surface-400 text-xs mt-4 flex items-center gap-1">
                        ...
                        หากรหัสลูกหนี้มีอยู่แล้ว ระบบจะปรับปรุงข้อมูลให้อัตโนมัติ
                    </p>
                </div>
            </div>
        </div>
    ...
    <!-- Confirm Dialog -->
    <DialogForm :confirmDialog="showConfirmDialog" :textContent="`ต้องการบันทึกนำเข้าลูกหนี้ ${importData.length} รายการ?`" confirmLabel="บันทึก (Enter)" @close="closeConfirmDialog" @confirm="confirmSave" />
</template>
