...
// Use composable for Debtor (Accounts Receivable)
const {
    // State
    reportData,
    currentPage,
    ...
    downloadPDF,
    initReport
} = useAccountStatusReport('receivable'); // 'receivable' = ลูกหนี้
...
<template>
    <div class="card bg-surface-card p-6 rounded-xl shadow-sm">
        ...
        <div class="mb-3 flex items-center justify-between">
            <div>
                <span class="text-xl font-medium text-surface-900 dark:text-surface-0">รายงานทางการเงิน / รายงานสถานะลูกหนี้</span>
        ...
        <div class="report-container">
            ...
            <div class="mb-4">
                ...
                <div class="flex justify-center items-center flex-col">
                    <div class="font-bold text-xl mb-0">รายงานสถานะลูกหนี้</div>
            ...
            <DataTable :value="reportData" showGridlines size="small" :rowHover="true">
                ...
                <Column field="customerid" header="รหัสลูกหนี้" style="width: 120px" :pt="{ headerCell: { style: 'text-align: center' } }">
                    <template #body="{ data }">
                        {{ data.customerid || '-' }}
                    </template>
                </Column>
                <Column field="customername" header="ชื่อลูกหนี้" style="min-width: 250px" :pt="{ headerCell: { style: 'text-align: center' } }">
                    <template #body="{ data }">
                        {{ data.customername || '-' }}
                    </template>
                </Column>
    ...
    <Popover ref="searchPopover">
        <div class="p-4 w-96">
            ...
            <div class="flex flex-col gap-3">
                ...
                <!-- Debtor Account -->
                <div>
                    <label class="block text-sm font-medium mb-1 text-surface-700 dark:text-surface-300">ลูกหนี้</label>
                    <Select v-model="selectedCustomer" :options="customers" optionLabel="displayLabel" :loading="customersLoading" placeholder="เลือกลูกหนี้..." filter showClear fluid />
                </div>
