...
// Use the shared withholding tax report composable for PND3 (Individual)
const {
    // State
    reportData,
    currentPage,
    ...
    downloadPDF,
    initReport
} = useWithholdingTaxReport(WITHHOLDING_TAX_TYPE.WHT, CUSTOMER_TYPE.INDIVIDUAL);
...
<template>
    <div class="card bg-surface-card p-6 rounded-xl shadow-sm">
        ...
        <div class="mb-3 flex items-center justify-between">
            <div>
                <span class="text-xl font-medium text-surface-900 dark:text-surface-0">รายงานภาษี / ภาษีหัก ณ ที่จ่าย (ภ.ง.ด.3)</span>
        ...
        <div class="report-container">
            ...
            <div class="mb-4">
                ...
                <div class="flex justify-center items-center flex-col">
                    <div class="font-bold text-xl mb-0">รายงานภาษีหัก ณ ที่จ่าย (ภ.ง.ด.3)</div>
            ...
            <DataTable v-model:expandedRows="expandedRows" :value="paginatedData" dataKey="docno" showGridlines size="small" :rowHover="true" @row-click="onRowClick">
                ...
                <Column field="custname" header="ชื่อผู้เสียภาษี" style="min-width: 180px">
                    <template #body="{ data }">
                        {{ data.custname || '-' }}
                    </template>
                </Column>
                <Column field="address" header="ที่อยู่ผู้เสียภาษี" style="min-width: 200px">
                    <template #body="{ data }">
                        {{ data.address || '-' }}
                    </template>
                </Column>
                ...
                <Column field="taxdocno" header="เลขที่หนังสือรับรอง" style="min-width: 130px">
                    <template #body="{ data }">
                        {{ data.taxdocno || '-' }}
                    </template>
                </Column>
                ...
                <Column field="taxdate" header="วันที่" style="width: 100px">
                    <template #body="{ data }">
                ...
                <Column header="จำนวนเงินที่จ่าย" style="width: 130px" headerClass="text-center" bodyClass="text-right">
                    <template #body="{ data }">
                        {{ formatCurrency(data.details?.[0]?.taxbase || 0) }}
                    </template>
                </Column>
                <Column header="ภาษีที่หักในครั้งนี้" style="width: 130px" headerClass="text-center" bodyClass="text-right">
                    <template #body="{ data }">
